...
rr = 1 - results/np.max(results) # normalizing this color map
plt.pcolormesh(Htest,Omegatest,results, norm= colors.LogNorm(), cmap='inferno')
plt.colorbar()
plt.contour(Htest,Omegatest,rr,[0.66,0.99],colors=['white'])
plt.contour(Htest,Omegatest,results,np.linspace(6.5,10.5,4),colors=['green'])
plt.contour(Htest,Omegatest,results,np.linspace(12,360,30),colors=['blue'])
plt.plot([Hprime], [Omegaprime], marker='o', markersize=3, color="red")
#
#plt.figure(3)#color map doesn't quite work
#plt.title(r'$X^2$')
#plt.xlabel(r'$H_0$')
#plt.ylabel(r'$\Omega_m$')
#plt.pcolormesh(Htest,Omegatest,results, norm= colors.LogNorm(), cmap='inferno')
#plt.colorbar()
#plt.contour(Htest,Omegatest,results,np.linspace(0,15,15),colors=['white'])
#plt.plot([Hprime], [Omegaprime], marker='o', markersize=3, color="red")
